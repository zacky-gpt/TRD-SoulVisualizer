...
const SAVE_KEY = 'trd_save_data_v12_hero';

// Job Data
const JOBS = [
    // Middle Path (Level based)
    { id:'novice', name:'Novice', req:null, bonus:{}, desc:"凡人", skill:{name:"Guts", type:"buff", cd:15, pwr:0, acc:1.0, desc:"食いしばり(2T)"} },
    { id:'veteran', name:'Veteran', req:{lv_min:10}, bonus:{phys:1.1, mag:1.1, def:0.9}, desc:"熟練者(全能微増)", priority:1, skill:{name:"Guts+", type:"buff", cd:15, pwr:0, acc:1.0, desc:"食いしばり(3T)+攻UP"} },
    { id:'hero', name:'Hero', req:{lv_min:20}, bonus:{phys:1.2, mag:1.2, def:0.8, eva:0.1}, desc:"英雄(全能強化)", priority:1, skill:{name:"Awakening", type:"buff", cd:20, pwr:0, acc:1.0, desc:"覚醒(4T不死+全強化)"} },

    // T-Axis
    { id:'warrior', name:'Warrior', req:{t_min:60}, bonus:{phys:1.2}, desc:"物攻+20%", skill:{name:"Braver", type:"phys", cd:10, pwr:2.5, acc:0.95, desc:"必殺の一撃"} },
    { id:'wizard', name:'Wizard', req:{t_max:40}, bonus:{mag:1.2}, desc:"魔攻+20%", skill:{name:"Fireball", type:"mag", cd:8, pwr:2.2, acc:1.0, desc:"大爆発"} },
    
    // D-Axis
    { id:'guardian', name:'Guardian', req:{d_min:60}, bonus:{def:0.8}, desc:"被ダメ-20%", skill:{name:"IronWall", type:"buff", cd:15, pwr:0, acc:1.0, desc:"完全防御(1T)"} },
    { id:'rogue', name:'Rogue', req:{d_max:40}, bonus:{eva:0.2}, desc:"回避+20%", skill:{name:"Mug", type:"phys", cd:6, pwr:1.0, acc:1.0, desc:"確定強奪"} },
    
    // R-Axis
    { id:'sniper', name:'Sniper', req:{r_min:60}, bonus:{crit:0.2}, desc:"Crit+20%", skill:{name:"Headshot", type:"phys", cd:8, pwr:1.8, acc:1.0, desc:"確定クリティカル"} },
    { id:'cleric', name:'Cleric', req:{r_max:40}, bonus:{heal:1.3}, desc:"回復+30%", skill:{name:"Pray", type:"heal", cd:12, pwr:0, acc:1.0, desc:"特大回復"} },
    
    // Hybrid (Priority 2)
    { id:'paladin', name:'Paladin', req:{t_min:60, d_min:60}, bonus:{phys:1.1, def:0.85}, desc:"物+10% 耐-15%", priority:2, skill:{name:"HolyBlade", type:"hyb", cd:10, pwr:2.2, acc:1.0, desc:"聖なる剣"} },
...
function updateJob() {
    ...
    for(let j of JOBS) {
        ...
        if(j.req && j.req.lv_min) {
            if(g.lv >= j.req.lv_min) { /* ok pass */ } else continue; // Skip if Lv not enough
        }
        ...
        // Priority rule: If balanced(Novice path), priority depends on level
        // If specialized(Warrior etc), priority is usually 2 or 3.
        // If 'Novice' path matched by level, we compare priority.
        if(p > bestP) { best=j; bestP=p; }
        else if(p === bestP) {
             // Same priority? Specialized usually wins over Novice if stats met
...
function consumeTime(v) {
    ...
    if(g.immortalTurns > 0) g.immortalTurns = Math.max(0, g.immortalTurns - v); // Guts
...
function actBattle(sk) {
    ...
    if(sk.id==='ironwall' || sk.id==='aegis') { log(`${sk.name}!`,"l-grn"); enemyTurn(true); updateUI(); return; } // Guard
...
function winBattle() {
    ...
    // Reset states
    g.awakening = false; g.isFocused = false; g.immortalTurns = 0;
...
window.showJobGuide = function() {
    ...
    }); document.getElementById('modal-job').style.display='flex'; // Change to flex
};
